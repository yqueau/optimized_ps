function M = make_gradient(mask)
	[Dup,Dum,Dvp,Dvm,Sup,Sum,Svp,Svm,Omega,index_matrix,imask] = gradient_operators(mask);
	[nrows,ncols] = size(mask);

	Du = Dup;
	no_bottom = find(~Omega(:,:,1));
	no_bottom = nonzeros(index_matrix(no_bottom));
	Du(no_bottom,:) = Dum(no_bottom,:);

	Dv = Dvp;
	no_right = find(~Omega(:,:,3));
	no_right = nonzeros(index_matrix(no_right));
	Dv(no_right,:) = Dvm(no_right,:);

	M = sparse([],[],[],2*size(Du,1),size(Du,2),2*length(imask));
	%~ M = zeros(2*size(Du,1),size(Du,2));
	M(1:2:end-1,:) = Du;
	M(2:2:end,:) = Dv;
end
